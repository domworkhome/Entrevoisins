...
public class FavNeighbourDetail extends AppCompatActivity{
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        fab.setOnClickListener(view -> {
            ...
            if (!mNeighbourApiService.getFavorites().contains(mNeighbour)) {
                ...
                fab.setImageResource(R.drawable.ic_star_yellorange_24dp);
                System.out.println(mNeighbour.getName() + " has been added to favorite list");
                ...
                Snackbar.make(mCoordinatorLayout, mNeighbour.getName() + " has been removed from your favorite list", Snackbar.LENGTH_SHORT).show();
            }
        });
    ...
    private void getIncomingIntent(){
        ...
        if(mIntent.hasExtra("neighbourName") && mIntent.hasExtra("neighbourAvatar") && mIntent.hasExtra("detailName") && mIntent.hasExtra("place") && mIntent.hasExtra("neighbourPhone") && mIntent.hasExtra("facebook") && mIntent.hasExtra("neighbourAbout")){
            ...
            String neighbourPic = mNeighbour.getAvatarUrl();

            neighbourDetailName = mNeighbour.getName();

            neighbourPlace = mNeighbour.getPlace();

            neighbourPhone = mNeighbour.getPhoneNumber();

            neighbourFacebook = mNeighbour.getFacebook();

            neighbourAbout = mNeighbour.getAbout();
